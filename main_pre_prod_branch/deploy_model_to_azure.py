...
def retrieve_latest_challenger_pre_test_model_from_unity_catalog():
    ...
    os.makedirs("./sim_rf_model_pre_prod", exist_ok=True)
    latest_model = mlflow.pyfunc.load_model(model_uri, dst_path="./sim_rf_model_pre_prod")
...
def deploy_model_to_workspace(model_uri):
    try:
        ...
        model_path = './sim_rf_model_pre_prod/artifacts/sim_rf_model_pre_prod_adls/model.pkl'

        # Registering the model
        model = Model.register(workspace=ws,
            model_name='sim_rf_model_pre_prod',
            model_path=model_path
            # model_path=model_uri
        )

        # Defining inference configuration
        env = Environment.from_conda_specification(name='.menv', file_path='main_preprod_branch/environment.yml')
        inference_config = InferenceConfig(entry_script='main_preprod_branch/score.py', environment=env)
...
model_uri = retrieve_latest_challenger_pre_test_model_from_unity_catalog()
